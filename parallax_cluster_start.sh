...
nohup parallax run -m "$MODEL" -n 3 --host 0.0.0.0 > /tmp/parallax_scheduler.log 2>&1 &
...
# Start Mac 2 (Worker) - reduced param_mem_ratio to avoid Metal OOM on 128GB
echo ""
echo "[2/4] Starting Mac 2 (Worker - 169.254.150.102, param_mem_ratio=0.2)..."
ssh humphrjk@169.254.150.102 "cd /Users/humphrjk/parallax && source venv/bin/activate && nohup parallax join --param-mem-ratio 0.2 > /tmp/parallax_worker.log 2>&1 &"
...
ssh humphrjk@169.254.150.106 "sudo docker run -d --gpus all --network host -v /home/humphrjk/.cache/huggingface:/root/.cache/huggingface gradientservice/parallax:latest-spark parallax join" && echo "  ✓ Docker container started" || echo "  ✗ Failed to start container"

# Start RR191562IP01 Docker container
echo ""
echo "[4/4] Starting RR191562IP01 (169.254.150.105)..."
ssh humphrjk@169.254.150.105 "sudo docker run -d --gpus all --network host -v /home/humphrjk/.cache/huggingface:/root/.cache/huggingface gradientservice/parallax:latest-spark parallax join" && echo "  ✓ Docker container started" || echo "  ✗ Failed to start container"
